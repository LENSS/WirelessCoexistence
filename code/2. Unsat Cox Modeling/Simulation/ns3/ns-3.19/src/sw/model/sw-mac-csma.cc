...
SwMacCsma::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SwMacCsma")
    ...
    .AddAttribute ("DifsTime",
                   ...
                   MakeTimeChecker ())
    .AddAttribute ("OS Delay",
                   "Application level packet delay",
                   TimeValue (MicroSeconds (300)),
                   MakeTimeAccessor (&SwMacCsma::OS_delay_wifi),
                   MakeTimeChecker ())
...
SwMacCsma::Enqueue (Ptr<Packet> packet, Mac48Address dest)
{
  ...
  m_cummQsize += m_pktQueue.size ();
  NS_LOG_UNCOND (Simulator::Now()<< " Packet "<< m_pktNum++ <<" Entered Queue (Size="<< m_pktQueue.size ()<<") at Wifi Node " << this->GetAddress()<< " Cml Pck Delay "<< m_cummDelay <<" Cml Q Size "<< m_cummQsize);
  ...
  if (m_state == IDLE)
    { 
      Simulator::Schedule (OS_delay_wifi, &SwMacCsma::CcaForDifs, this); // Added OS delay
      m_sendTime = Simulator::Now() + OS_delay_wifi;
    }
...
SwMacCsma::CcaForDifs ()
{
  ...
  //NS_LOG_FUNCTION (this << "TIME NOW : -------- " << now);
  ...
  if (m_backoffRemain == Seconds (0))
    {
      ...
      NS_LOG_DEBUG ("Select a random number (0, " << m_cw - 1 << ") " << cw << 
                    " backoffRemain " << m_backoffRemain << " will finish " << m_backoffRemain + Simulator::Now () << " time now " << Simulator::Now() << this->GetAddress());
    }
  ...
  if (m_backoffTimeoutEvent.IsRunning ())
    {
      ...
      NS_LOG_DEBUG("Freeze backoff! Remain " << m_backoffRemain << this->GetAddress());
    }
  CcaForDifs ();
}
void
...
SwMacCsma::SendData ()
{
  ...
  //NS_LOG_DEBUG ("Sending Packet Number " << m_pktNum++ <<" Node : " <<this->GetAddress() << " Time: " << Simulator::Now());
...
SwMacCsma::SendAck (Mac48Address dest)
{
  ...
  SendPacket (packet, 1); // Changed to 1 to use data rate instead of basic rate
}
bool 
...
{
  m_sequence++;
  m_cummDelay += (Simulator::Now() - m_sendTime);
  if (success)
    {
      NS_LOG_UNCOND (Simulator::Now()<< " Success to transmit Packet " << m_pktNumSuccess++ << " at Node Address : " << this->GetAddress());
      m_traceSendDataDone (m_device->GetNode ()->GetId (), m_device->GetIfIndex (), true);
    }
  else
    {
      NS_LOG_UNCOND (Simulator::Now()<< " Fail to transmit DATA!");
  ...
  SetCw (m_cwMin);
  Simulator::Schedule (OS_delay_wifi, &SwMacCsma::CcaForDifs, this); // Added OS delay for each packet
  m_sendTime = Simulator::Now()+OS_delay_wifi;
}
void
SwMacCsma::StartOver ()
