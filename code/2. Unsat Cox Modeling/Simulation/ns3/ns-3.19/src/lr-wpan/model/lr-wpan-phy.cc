...
LrWpanPhy::StartRx (Ptr<SpectrumSignalParameters> spectrumRxParams)
{
  ...
  // Added check for collision detection 
  ...
  if (lrWpanRxParams != 0 )
  {
      ...
      // StartRx() and EndRx() states.
        if(!m_EndCTRXRequest.IsRunning())
        {
                if(m_trxState == IEEE_802_15_4_PHY_BUSY_TX)
                {
                        ...
                        m_trxState = IEEE_802_15_4_PHY_COLLISION;
                }     
                else
                {
			...
			NS_LOG_DEBUG (this << "*****************COLLISSION HAPPENED ********************");
                ...
                m_EndRxRequest = Simulator::Schedule (duration, &LrWpanPhy::EndRx, this);
                m_phyRxBeginTrace (p);  
        }
        else
        {
                ...
                if(lrWpanRxParams->duration > duration)
                {
                        ...
                        m_EndCTRXRequest = Simulator::Schedule (duration, &LrWpanPhy::ChangeTrxState, this, IEEE_802_15_4_PHY_RX_ON);
                }
                return;
        }
    }
    ...
    {
      NS_LOG_DEBUG (this << "WIFI transmission going on, BUSY channel");
      m_trxState = IEEE_802_15_4_PHY_BUSY;
      if(m_EndRxRequest.IsRunning() || m_EndCTRXRequest.IsRunning())
      {      
                m_trxState = IEEE_802_15_4_PHY_COLLISION;
		NS_LOG_DEBUG (this << "*********COEXISTENCE COLLISSION HAPPENED ********************");
                ...
                {
                        //Simulator::Schedule (duration, &LrWpanPhy::ChangeTrxState, this, IEEE_802_15_4_PHY_RX_ON);
                        return;                        
                }
       } 
       else
...
void LrWpanPhy::EndRx ()
{
  ...
  if (m_currentRxPacket.second == true)
    ...
    {
      //NS_LOG_DEBUG ("SIGNAL OK - WITH ERROR MODEL");
      ...
      if (m_random.GetValue () > per && m_trxState == IEEE_802_15_4_PHY_BUSY_RX)
        {
  ...
  if (m_trxState == IEEE_802_15_4_PHY_TX_ON)
    {
      ...
      NS_LOG_DEBUG (this << "DURATION FOR TRANSMISSION" << txParams->duration << " Time Now : " << Simulator::Now());
...
{
  NS_LOG_DEBUG (this << " Time Now : " << Simulator::Now());
  if (m_trxState == IEEE_802_15_4_PHY_RX_ON)
    {
      //call StartRx or ED and then get updated m_rxTotalPower in EndCCA?
      Time ccaTime = MicroSeconds(0);//Seconds (8.0 / GetDataOrSymbolRate (false));
      NS_LOG_DEBUG (this << "CCA TIME----------------" << ccaTime );
...
LrWpanPhy::EndTx ()
{
  ...
  if(duration > txDuration)
  {
          ...
          Simulator::Schedule (duration-txDuration, &LrWpanPhy::ChangeTrxState, this, IEEE_802_15_4_PHY_RX_ON);
          txDuration = Seconds(0);
          return;
  }
  if ((m_currentTxPacket.second == false) && (m_trxState == IEEE_802_15_4_PHY_BUSY_TX))
    {
      NS_LOG_DEBUG (Simulator::Now()<< " Packet successfully transmitted");
    ...
    {
      NS_LOG_UNCOND (Simulator::Now()<< " Packet transmission aborted - COLLISION (LrWpan)");
      ...
      if (!m_pdDataConfirmCallback.IsNull ())
        {
          ...
          ChangeTrxState (IEEE_802_15_4_PHY_BUSY_TX);
          m_pdDataConfirmCallback (IEEE_802_15_4_PHY_SUCCESS);//m_trxState);
        }
    }
  txDuration = Seconds(0);
  m_currentTxPacket.first = 0;
