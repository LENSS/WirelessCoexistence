...
LrWpanCsmaCa::LrWpanCsmaCa ()
{
  ...
  m_macMinBE = 3;
  m_macMaxBE = 80;
  m_macMaxCSMABackoffs = (uint64_t) 1000000000;
...
void
LrWpanCsmaCa::setMacBE (uint8_t macBE)
{
  NS_LOG_FUNCTION (this << macBE);
  m_BE = macBE;
}

uint8_t
LrWpanCsmaCa::getMacBE (void) const
{ 
  NS_LOG_FUNCTION (this);
  return m_BE;
}
...
{
  NS_LOG_DEBUG (this << "Time : " << Simulator::Now());
  ...
  if (isSlottedCsmaCa ())
    {
      ...
      if (m_BLE)
        {
          //m_BE = MIN (2, m_macMinBE); -> original implementation
          m_BE = (uint8_t) 320;
        }
      else
        {
          //m_BE = m_macMinBE; -> original implementation
          m_BE = (uint8_t) 320;
        }
      //TODO: for slotted, locate backoff period boundary. i.e. delay to the next slot boundary
      backoffBoundary = getTimeToNextSlot ();
      Simulator::Schedule (MicroSeconds (backoffBoundary),&LrWpanCsmaCa::RandomBackoffDelay,this);
    }
  else
    {
      m_BE = m_macMinBE;
...
//  uint64_t upperBound = (uint64_t) pow (2, m_BE) - 1; -> original implementation
  uint64_t upperBound = (uint64_t) (m_BE) - 1;
  ...
  uniformVar = UniformVariable (10, upperBound);
  ...
  if (isUnSlottedCsmaCa ())
    ...
    {
      NS_LOG_DEBUG ("Slotted:  proceeding after backoff of " << randomBackoff.GetMicroSeconds () << " us" << " Time Now : " << Simulator::Now());
  ...
  if (canProceed)
    {
      ...
      backoffBoundary = getTimeToNextSlot ();
      Simulator::Schedule (MicroSeconds (backoffBoundary),&LrWpanCsmaCa::RequestCCA,this);
    }
  else
    {
...
LrWpanCsmaCa::PlmeCcaConfirm (LrWpanPhyEnumeration status)
{
  ...
  if (status == IEEE_802_15_4_PHY_IDLE)
      ...
      if (isSlottedCsmaCa ())
        {
          ...
          if (m_CW == 0)
            ...
            {
              NS_LOG_DEBUG ("Perform CCA again, m_CW = " << static_cast<uint16_t> (m_CW) << " Time Now : " << Simulator::Now());
  ...
  else
    {
      ...
      //m_BE = MIN (m_BE + 1, m_macMaxBE); -> original implementation
      m_BE = (uint8_t) m_macMaxBE;
      m_NB++;
