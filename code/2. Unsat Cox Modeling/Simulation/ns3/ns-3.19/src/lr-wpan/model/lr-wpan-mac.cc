...
LrWpanMac::DoDispose ()
{
  ...
  m_txQueue.clear ();
  m_phy = 0;
  m_mcpsDataIndicationCallback = MakeNullCallback< void, McpsDataIndicationParams, Ptr<Packet>, uint32_t > ();
...
LrWpanMac::McpsDataRequest (McpsDataRequestParams params, Ptr<Packet> p)
{
  ...
  txQElement->txQPkt = p;
  m_cummQsize += m_txQueue.size();
  NS_LOG_UNCOND (Simulator::Now()<< " Packet "<< m_pktNum++ <<" Entered Queue (Size="<< m_txQueue.size() <<") at Zigbee Node "<<this->GetShortAddress() << " Cml Pck Delay "<< m_cummDelay <<" Cml Q Size "<< m_cummQsize);
  ...
  if (m_txQueue.empty ())
    {
      ...
      //Simulator::ScheduleNow (&LrWpanCsmaCa::Start, m_csmaCa); -> Original Implementation
      Simulator::Schedule (OS_Delay_lrwpan, &LrWpanCsmaCa::Start, m_csmaCa);
      m_sendTime = Simulator::Now() + OS_Delay_lrwpan;
    }
  else
    {
      ...
      if (!m_mcpsDataIndicationCallback.IsNull ())
        {
          ...
          m_mcpsDataIndicationCallback (params, p, m_pktNumSuc++);
        }
      else
        {
      ...
      if (!m_txQueue.empty ())
        {
          ...
          //Simulator::ScheduleNow (&LrWpanCsmaCa::Start, m_csmaCa); -> original 
          Simulator::Schedule(OS_Delay_lrwpan,&LrWpanCsmaCa::Start, m_csmaCa);
          m_sendTime = Simulator::Now() + OS_Delay_lrwpan;
        }
    }
  ...
  if ((m_lrWpanMacState == CHANNEL_IDLE) && (m_txPkt))
    {
      ...
      NS_LOG_DEBUG ( this << " channel idle " << Simulator::Now());
