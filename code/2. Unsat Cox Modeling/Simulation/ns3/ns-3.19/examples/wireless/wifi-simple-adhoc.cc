                             ...
                             uint32_t pktCount, Time pktInterval)
{
  if (pktCount > 0)
    {
      socket->SendTo (Create<Packet> (pktSize), 0, Address(InetSocketAddress(Ipv4Address("10.1.1.1"),80)));
                             ...
                             uint32_t pktCount, Time pktInterval)
{
  if (pktCount > 0)
    {
      socket->SendTo (Create<Packet> (pktSize), 0, Address(InetSocketAddress(Ipv4Address("10.1.1.2"),80)));
...
//    wifi.EnableLogComponents ();  // Turn on all Wifi logging
  ...
  InetSocketAddress local1 = InetSocketAddress (i.GetAddress(1), 80);
  ...
  InetSocketAddress remote1 = InetSocketAddress (i.GetAddress(3), 80);
  ...
  NS_LOG_UNCOND ("Testing (4-node) scenario " << numPackets  << " packets sent with receiver rss " << rss );

  Simulator::ScheduleWithContext (source->GetNode ()->GetId (),
                                  Seconds (1.0), &GenerateTraffic1, 
