...
% n=3;
% clear all;
function ret = with_timing(T, WIFI_START, WIFI_END, BMAC_START, BMAC_END, aMinBE, aMaxBE, Packet_w, Wi_b, Wc_b, Packet_b)
...
Ptr_ws(1:totalRuns)=0;
Ptr_bs(1:totalRuns)=0;
Psucc_ws(1:totalRuns)=0;
Psucc_bs(1:totalRuns)=0;
...
% outname=['varying_wifi-', num2str(zw_time(2)),'-', num2str(zw_time(3)),'-', ...
...
%%%%%%%%%Comment below and uncomment above for varying WiFi parameters
% %%%%%%%%%Uncomment below and comment above for varying BMAC parameters
% outname=['varying_bmac-', num2str(zw_time(2)),'-', num2str(zw_time(3)),'-', ...
%     num2str(ceil(zw_time(1))),'-',num2str(zw_time(4)), ...
%     '-', num2str(zw_time(5)),'-', num2str(zw_time(6)), '.txt'];
% diary (outname)
% for k_b=6:-1:2  %%%%%Eplb = 20*k_b + 8
% for j_b=[4,2]%4:-1:2  %%%%%Wc = 20*j_b
% for i_b=3:-1:1  %%%%%Wi = 80*i_b
% for i_w=3:3  %%%%%Eplw = 500*i_w
% for j_w=5:5 %%%%%m0
% for k_w=10:10 %%%%%m+m0
% %%%%%%%%%%Uncomment below for default parameters and varying number of
%%%%%%%%%%nodes. Comment all above
...
% BMAC_START=5;
% BMAC_END=30;
% outname=['varying_num_nodes-', num2str(zw_time(2)),'-', num2str(zw_time(3)),'-', ...
%     num2str(ceil(zw_time(1))),'-',num2str(zw_time(4)), ...
%     '-', num2str(zw_time(5)),'-', num2str(zw_time(6)), '.txt'];
% diary (outname)
% for k_b=6:6  %%%%%Eplb
...
if(N_w==0)
    disp('Calling only BMAC');
    xvect = fsolve('only_bmac', xguess);
    alpha = xvect(2);
    Pc=0;
    Pf=0;
    %b000 = xvect(1);
else
    xvect = fsolve('with_timing_math', xguess);
    alpha = xvect(2);
    Pc = xvect(1);
    Pf = xvect(3);
end
...
Tr = ceil(Epl_w*8/channel_bit_rate/10);
Trb = ceil((Epl_b)*8*4/30);
Lbt = ceil((Epl_b+phy_b)*8*4/30);
...
if(N_w==0)
    ...
    b000 = 1 / (3*((Wi+1)/2+(Wc+1)*x/(2*(1-x))+(1-alpha)/(1-x))+ 7*(1-alpha)/(1-x)+3*Lbt+3*Losb);%
    tau_b = b000;
    Ptrw = Lsw*(1-Pf)*b001+Lcw*(1-Pf)*Pc*b001/(1-Pc);
    Ptrb = 3*Lbt*b000;
...
% Tbo_w = 10;
% 
% Tpl_w = Ts_w;%/(Ts_w+SIFS+PROP+ACK+DIFS+PROP+ Tosd_w);
% 
% Throughput_w = Ptr_w*Psucc_w*Tpl_w/(Ptr_w*Psucc_w*Tsucc_w+Ptr_w*Pcolli_w*Tcolli_w+ P_idle*Tbo_w);%Ptr_w*Posd_w*Tosd_w);
% Actual_throughput_w = Ptr_w*Psucc_w*(Tpl_w*channel_bit_rate)/(Ptr_w*Psucc_w*Tsucc_w+Ptr_w*Pcolli_w*Tcolli_w+P_idle*Tbo_w);%Pbo_w*Tbo_w+ Pos_w*Tosd_w);%Ptr_w*Posd_w*Tosd_w);
Ps_w = Tr*(1-Pf)*b001;
Ptr_w = 1-((1-Ptrw)^N_w);%*(1-tau_b)^N_b;
Psucc_w = N_w*Ps_w*((1-Ps_w)^(N_w-1))*((1-Ptrb)^N_b)/Ptr_w;
% Pcolli_w = 1-Psucc_w;
% 
% %Pbo_w = (1-Ptrw-Posw)^N_w;
% %Pos_w = 1-((1-tau_w)^N_w);
...
Throughput_w = wm_factor_wifi*Ptr_w*Psucc_w;%*Tpl_w/(Ptr_w*Psucc_w*Tsucc_w+Ptr_w*Pcolli_w*Tcolli_w+ P_idle*Tbo_w);%Ptr_w*Posd_w*Tosd_w);
Actual_throughput_w = wm_factor_wifi*Ptr_w*Psucc_w*channel_bit_rate;%*(Tpl_w*channel_bit_rate)/(Ptr_w*Psucc_w*Tsucc_w+Ptr_w*Pcolli_w*Tcolli_w+P_idle*Tbo_w);%Pbo_w*Tbo_w+ Pos_w*Tosd_w);%Ptr_w*Posd_w*Tosd_w);
...
% 
% Actual_throughput_b = Ptr_b*Psucc_b*(Tpl_b*250)/(Ptr_b*Psucc_b*Tsucc_b+Ptr_b*Pcolli_b*Tcolli_b+Pbo_b*Tbo_b);
Ps_b = (((1-tau_w)^(N_w))*((1-tau_b)^(N_b-1)))*3*Trb*tau_b;
Ptr_b = 1-(1-Ptrb)^N_b;%*(1-tau_w)^N_w;
Psucc_b = N_b*Ps_b;%*((1-Ptrb)^(N_b-1))*((1-Ptrw)^N_w)/Ptr_b;
% Pcolli_b = 1-Psucc_b; %???
% Posd_b = Psucc_b+Pcolli_b; %???
% Pbo_b = 1-Ptr_b;%(1-Ptrb-Posb)^N_b;

disp(['  Ptr_b = ', num2str(Ptr_b) ])
disp(['  Ps_b = ', num2str(Ps_b) ])
disp(['  Psucc_b = ', num2str(Psucc_b) ])
% disp(['  Pcolli_b = ', num2str(Pcolli_b) ])
% disp(['  Posd_b = ', num2str(Posd_b) ])
% disp(['  Pbo_b = ', num2str(Pbo_b) ])
...
% Tpl_b = Ts_b;%(Epl_b)*8*4;
if(N_w==0)
    Throughput_b = wm_factor_bmac*Ptr_b*Psucc_b;%*Tpl_b/(Ptr_b*Psucc_b*Tsucc_b+Ptr_b*Pcolli_b*Tcolli_b+Pbo_b*Tbo_b);
else
    Throughput_b = wm_factor_bmac*Ptr_b*Psucc_b;%*Tpl_b/(Ptr_b*Psucc_b*Tsucc_b+Ptr_b*Pcolli_b*Tcolli_b+Pbo_b*Tbo_b);
end
Actual_throughput_b = wm_factor_bmac*Ptr_b*Psucc_b*250;%*(Tpl_b*250)/(Ptr_b*Psucc_b*Tsucc_b+Ptr_b*Pcolli_b*Tcolli_b+Pbo_b*Tbo_b);
...
Ptr_ws(runNo)=Ptr_w;
Ptr_bs(runNo)=Ptr_b;
Psucc_ws(runNo)=Psucc_w;
Psucc_bs(runNo)=Psucc_b;
